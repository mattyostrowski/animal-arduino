...
void printStatus() {
  ...
  Serial.print("Pin ON debounce ms="); Serial.println(pinOnDebounceMs);
  Serial.print("Pin OFF debounce ms="); Serial.println(pinOffDebounceMs);
  Serial.println();
}
...
void handleCmd(String s) {
  ...
  if (lower.startsWith("pinondebounce ")) {
  unsigned long v = (unsigned long) max(0, lower.substring(14).toInt());
  pinOnDebounceMs = v;
  Serial.print("Pin ON debounce set to ");
  Serial.print(pinOnDebounceMs);
  Serial.println(" ms");
  return;
}

if (lower.startsWith("pinoffdebounce ")) {
  unsigned long v = (unsigned long) max(0, lower.substring(15).toInt());
  pinOffDebounceMs = v;
  Serial.print("Pin OFF debounce set to ");
  Serial.print(pinOffDebounceMs);
  Serial.println(" ms");
  return;
}
...
void printCommands() {
  ...
  Serial.println("  pinOnDebounce ms");
  Serial.println("  pinOffDebounce ms");
...
void loop() {
  ...
  unsigned long requiredDebounce = (reading == LOW) ? pinOnDebounceMs : pinOffDebounceMs;
  if ((nowLoop - lastChangeTime[i]) >= requiredDebounce && reading != stableState[i]) {
